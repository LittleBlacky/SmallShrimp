...
```
Round  改动                        旧 recall  新 recall
────────────────────────────────────────────────────────
  0    keyword baseline              88%        35%
  1    首次 HRR (空格分词)           44%        —
  2    字符级中文 tokenization       94%        —
  3～6  优化/缓存                    94%        —
  7    TF 加权                      88% ❌     42% ✅
     ────────────────────────────────────────────────
  新    HRR (TF) 在大数据集上          —         42% ✅
...
**最终结论（449 条记忆 / 100 条语义查询）**：

| 策略 | recall@5 | 耗时 | 比关键词 |
|------|----------|------|---------|
| keyword-only | 35% | 5.4s | baseline |
| HRR (等权) | 39% | **1.8s** | +4%, 3x 快 |
| **HRR (TF)** | **42%** | **1.8s** | **+7%, 3x 快** |
| hybrid (0.1+0.9) | **43%** | 6.3s | +8% |

HRR 在真实语义查询集上全面超越关键词，速度快 3 倍。TF 加权在大数据集下优于等权。
