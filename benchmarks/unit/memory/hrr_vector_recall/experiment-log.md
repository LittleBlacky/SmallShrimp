...
**目的**：用更大、更真实的数据验证各策略的差距。

**改动**：
- 记忆从 15 条扩充到 **449 条**（5 层全覆盖）
- 查询从 15 条扩充到 **100 条**
- 查询从关键词倾向改为**自然语义提问**
...
策略                     recall@5       耗时
───────────────────────────────────────────────
keyword-only              35% (44/124)   5.4s
hrr (等权)                39% (48/124)   1.8s
hrr (TF)                  42% (52/124)   1.8s   ← 最佳单策略
hybrid (0.1kw+0.9hrr)     43% (53/124)   6.3s   ← 最优融合
```

**分析**：

1. **关键词从 88% 掉到 35%** — 语义查询不再是关键词能蒙对的
2. **HRR 首次全面超越关键词** — 等权 39%、TF 42% > 关键词 35%
3. **TF 加权在大数据集上优于等权** — 与小数据集结论相反，降权高频词后低频 token 信号更清晰
4. **HRR 速度优势明显** — 1.8s vs 关键词 5.4s

**结论**：大数据集验证了 HRR 是关键词的有效替代——recall 更高、速度更快。TF 加权在大数据集下优于等权。
...
```
dim=128  35%  dim=256  39%  dim=512  39%
dim=1024 39%  dim=2048 40%
```
结论：维度对 recall 影响小，1024 是性价比高点。

### 结果级融合

keyword top-10 + HRR top-10 合并取 top-5。结果：39%，不如分数级融合。

### 查询压缩 + TF 加权

去掉自然问句中的话术（"应该怎么"、"要怎么"、"有什么"），压缩后 HRR 编码：
...
两步法：先跑 HRR (压缩+TF) 基线找到 48 个漏报，只对这 48 个调 LLM 生成同义改写。

```
策略                           recall@5        耗时
───────────────────────────────────────────────────────
keyword-only                     36%           4.4s
HRR (压缩+TF)                    43%           1.2s
HRR (压缩+TF) + LLM扩展漏报       51% ✅        62.5s
```
