...
SmallShrimp is a Python 3.11+ multi-endpoint, continuously evolving personal assistant Agent packaged from `src/SmallShrimp`. It is not positioned as a generic agent framework. Its product stages are: first, help users complete tasks; second, learn task scenes, user profile, preferences, and reusable methodology until it approximates a user clone; third, self-evolve beyond the user's baseline through reflection, failure learning, and skill consolidation; fourth, feed improvements back to the user so the human and agent improve together. Core assistant behavior lives in `src/SmallShrimp/core`, CLI entry points in `src/SmallShrimp/cli`, provider integrations in `src/SmallShrimp/provider`, channel adapters in `src/SmallShrimp/channels`, server workers in `src/SmallShrimp/server`, and built-in tools in `src/SmallShrimp/tools`. Tests are flat pytest modules in `tests/` named by feature, such as `test_memory.py` and `test_tool_registry.py`. Runtime examples and user data live under `workspace/`, including `agents/`, `skills/`, `crons/`, `sessions/`, and `memories/`.
